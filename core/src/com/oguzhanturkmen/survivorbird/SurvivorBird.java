package com.oguzhanturkmen.survivorbird;
		...
		if (gameState == 1) {
			...
			for (int i = 0; i < numberOfEnemies; i++) {
				if (enemyX[i] < Gdx.graphics.getWidth() / 15) {
					...
					enemyX[i] = enemyX[i] - enemyVelecotiy;
				}
				enemyX[i] = enemyX[i] - enemyVelecotiy;
			...
			font.draw(batch,"Game Over! Tap to play again!",100,Gdx.graphics.getHeight()/2);
			if (Gdx.input.justTouched()) {
				gameState = 1;
